...
class InvariantNet(nn.Module):
    def __init__(self, num_samples, h=64, k=16, q=2.0, device='cpu'):
        super().__init__()
        self.sampler = InvariantSpiralSampler(num_samples=num_samples, q=q, device=device)
        self.path = nn.Sequential(
            SpectralLayer(num_samples, h, k=k),
            nn.ReLU(),
            nn.Linear(h, 10)
        )
    def forward(self, x):
...
if __name__ == "__main__":
    ...
    for ns in n_samples:
        for q in q_values:
            ...
            model = InvariantNet(num_samples=ns, q=q, h=64, device=device).to(device)
            optimizer = optim.Adam(model.parameters(), lr=0.002)
            criterion = nn.CrossEntropyLoss()
            
            # Train (3 epochs for speed in sweep)
    ...
    print("\n" + "="*60)
    print(f"{'NS':<6} | {'q':<6} | {'0°':<8} | {'15°':<8} | {'30°':<8} | {'45°':<8}")
    print("-" * 60)
    for r in sweep_results:
        print(f"{r['ns']:<6} | {r['q']:<6} | {r['acc_0']:<8.2f} | {r['acc_15']:<8.2f} | {r['acc_30']:<8.2f} | {r['acc_45']:<8.2f}")
