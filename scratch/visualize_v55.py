...
class LineNeuronLayer(nn.Module):
    def __init__(self, num_neurons=256):
        ...
        self.points = nn.Parameter(torch.zeros(num_neurons, 2, 2))
        self.log_sigma_pos = nn.Parameter(torch.zeros(num_neurons))
        self.log_sigma_neg = nn.Parameter(torch.zeros(num_neurons))
...
def visualize():
    ...
    fig.suptitle("Symmetry Break Test (v55) - Initialized as same vertical line", fontsize=16)
    for i in range(256):
        ax = axes[i // 16, i % 16]
    ...
    plt.savefig("v55_symmetry_gallery.png", dpi=150)
    print("🎨 Gallery saved as v55_symmetry_gallery.png")
    plt.show()
