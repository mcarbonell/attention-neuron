...
==============================================
CERTIFIED V305 MQAR BENCHMARK: ISO-MEMORY CONTROL AUDIT.
...
- Optimization: AdamW (lr=3e-3, weight_decay=0.0, steps=1500, batch_size=32)
- Evaluates:
...
def print_log_header(device_str):
    ...
    print(f" Numerical Stabilizers:  solve_triangular, grad_norm_clipping(1.0), AdamW(lr=3e-3, wd=0.0)", flush=True)
    ...
    print("   - Optimizer: AdamW | Learning Rate: 3.00e-03 | Weight Decay: 0.00e+00", flush=True)
    print("   - Sequence Specs: seq_len = 128 | n_pairs = 29 | Batch Size = 32 | Steps = 1500", flush=True)
...
def evaluate_model_on_certified_harness(block_cls, d_k, name, steps=1500, n_pairs=29, seq_len=128, seeds=[42, 43]):
    device_str = "cuda" if torch.cuda.is_available() else "cpu"
    device = torch.device(device_str)
    print(f"\n{ts()} --- Certified v305 Training {name} (d_k={d_k}, d_model=128, n_layers=4, steps={steps}, lr=3e-3) ---", flush=True)
    accs = []
    ...
    for seed in seeds:
        ...
        for step in range(1, steps + 1):
            ...
            if torch.isnan(loss):
                print(f"{ts()}  [WARNING] Seed {seed} Step {step}: NaN Loss detected, skipping!", flush=True)
                continue
            ...
            # Print progress every 150 steps for ALL seeds
            if step % 150 == 0 or step == 1 or step == steps:
                with torch.no_grad():
                    ...
                    print(f"{ts()}  [Seed {seed} Step {step:>4}/{steps}] Loss: {loss.item():.4f} | Dynamic MQAR Acc: {step_acc:.2f}%", flush=True)
        ...
        with torch.no_grad():
            ...
            dt_seed = time.time() - t_start_seed
            print(f"{ts()}  [Seed {seed} Final Val Accuracy]: {final_acc:.2f}% (Seed Time: {dt_seed:.2f}s)", flush=True)
