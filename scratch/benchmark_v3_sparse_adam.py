...
class SparseAttentionLayer(nn.Module):
    ...
    def get_l1_penalty(self):
        # Calculamos la penalización L1 sobre los factores multiplicativos
        # para forzar esparcidad en la modulación topológica.
        return torch.sum(torch.abs(self.delta_in_m)) + torch.sum(torch.abs(self.delta_out_m))

class SparseAttentionMLP(nn.Module):
    def __init__(self, mask_prob=0.5):
        super().__init__()
        self.layer1 = SparseAttentionLayer(784, 512, rank=2, mask_prob=mask_prob)
        self.layer2 = SparseAttentionLayer(512, 10, rank=2, mask_prob=mask_prob)
    ...
    def get_l1_penalty(self):
        return self.layer1.get_l1_penalty() + self.layer2.get_l1_penalty()

def main():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Benchmarking V3 (Sparse Attention - L1 Regularization) with ADAM on: {device}")

    BATCH_SIZE = 256
    EPOCHS = 10
    LR = 0.001
    L1_LAMBDA = 1e-4 # Factor de regularización L1
    ...
    model = SparseAttentionMLP(mask_prob=0.5).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # Loss principal + Penalización L1 sobre la modulación
