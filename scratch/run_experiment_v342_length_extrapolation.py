...
def train_model(model, train_loader, val_dataset, epochs=25, lr=2e-3, device='cpu'):
...
def run_experiment_v342():
    print("=" * 80)
    print(" EXPERIMENTO V342: GENERALIZACIÓN A LONGITUD EXTREMA EN ZERO-SHOT (ROBUSTO)")
    print("=" * 80)
    ...
    print(f"\n[Config v342]")
    print(f" * Entrenando SOLO en Secuencias de Longitud L_train = {L_train}")
    print(f" * Muestras: {num_samples_train} | Épocas: {epochs} | LR: 2e-3 Cosine")
    ...
    trained_models = {}
    print("--- FASE 1: ENTRENAMIENTO HASTA CONVERGENCIA EN L = 256 ---")
    for name, model in models.items():
        print(f"\n[Entrenando] {name} en L={L_train}...")
        t0 = time.time()
        model = train_model(model, train_loader, val_dataset_256, epochs=epochs, lr=2e-3, device=device)
    ...
    for L_eval in eval_lengths:
        ...
        for name, model in trained_models.items():
            ...
            print(f"   * {name:30s} | Accuracy en L={L_eval:4d}: {acc:6.2f}%")

    print("\n" + "=" * 80)
    print("          RESUMEN DE GENERALIZACIÓN A LONGITUD EXTREMA (V342)")
    print("=" * 80)
