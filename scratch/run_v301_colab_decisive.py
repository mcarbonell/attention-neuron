"""
run_v301_colab_decisive.py
==========================
Self-contained script to run V301 Decisive Benchmark in Kaggle / Colab (Free GPU).

Adds Candidate Arm 3: RealDeltaNetRectangular (K_dim=128, V_dim=64 -> 8192 floats).
Fixes RNG dataset sharing and Val/Test split.
"""
...
CFG = {
    ...
    "d_k_list": [64], # Focus on d_k=64 for decisive test
    "iso_floats_map": {
...
def generate_mqar_dataset(num_batches, batch_size, num_pairs, seq_len, seed=42, device=device):
    ...
    for _ in range(num_batches):
        x, y = generate_mqar_batch(batch_size, num_pairs, seq_len, rng, device=device)
...
class ComplexDeltaPhaseHolographicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return res + self.out_proj(retrieved) + self.ffn(self.norm2(res + self.out_proj(retrieved)))
...
print("Starting V301 DECISIVE BENCHMARK on GPU...", flush=True)
results_matrix = {}
for d_k in CFG["d_k_list"]:
    ...
    for num_pairs in CFG["num_pairs_list"]:
        seq_len = 8 * num_pairs
        print(f"\n  >>> Load: {num_pairs} Pairs (L={seq_len}) <<<", flush=True)
        for name, block_cls, block_kwargs in model_specs:
            ...
            for lr in CFG["lr_grid"]:
                ...
                val_acc, test_acc, eval_t = train_and_eval(model, name, num_pairs, seq_len, lr, epochs=CFG["epochs"], steps_per_epoch=CFG["steps_per_epoch"])
                total_time += eval_t
