...
def print_architecture_inventory(name, model):
    ...
    for idx, (layer_name, module) in enumerate(model.named_children()):
        ...
        log_print(f" Capa {idx}: {layer_name:<15s} | Clase: {module.__class__.__name__:<40s} | Params: {mod_params:,}")
    log_print("---------------------------------------------------------")
...
def train_model(name, model, train_loader, val_dataset, epochs=30, lr=3e-3, device='cpu'):
...
def run_experiment_v347():
    log_print("================================================================================")
    log_print(" EXPERIMENTO V347: VECTORIZACIÓN PARALELA IIR SCAN Y ACELERACIÓN WALL-CLOCK")
    log_print("================================================================================")
    ...
    log_print(f"ID Experimento: v347")
    ...
    # REGLA DE ORO (Primero el Candidato Vectorizado v347):
    models = {
        "Vectorized Selective-Conv1D IIR (v347 Candidato)": VectorizedSelectiveConv1DIIRTransformerV347(
            vocab_size=config["vocab_size"],
        ...
        "Selective-Conv1D IIR (v346 Bucle Python)": SelectiveConv1DIIRTransformerV346(
            vocab_size=config["vocab_size"],
            d_model=config["d_model"],
            d_state=64
        )
    ...
    log_print("--- FASE 1: ENTRENAMIENTO MQAR EN L = 128 (30 ÉPOCAS) ---")
    for name, model in models.items():
        log_print(f"\n[Entrenando] {name}...")
        res = train_model(
    ...
    for L_eval in config["eval_lengths"]:
        ...
        for name, data in trained_results.items():
            ...
            log_print(f"   * {name:52s} | Acc L={L_eval:4d}: {acc:6.2f}%")

    log_print("\n================================================================================")
    log_print("          RESUMEN EXPERIMENTO V347 (VECTORIZACIÓN & SPEEDUP)")
    log_print("================================================================================")
    log_print(f"{'Modelo':52s} | {'Wall Clock (s)':14s} | {'L=128 (Train)':13s} | {'L=256':9s} | {'L=512':9s}")
    log_print("-" * 105)
    ...
    for name in models.keys():
        ...
        log_print(f"{name:52s} | {wall_t:13.2f}s | {accs[0]:12.2f}% | {accs[1]:8.2f}% | {accs[2]:8.2f}%")
    ...
    json_path = os.path.join(results_dir, "v347_results.json")
    ...
    # LEDGER
    cand_acc = eval_results["Vectorized Selective-Conv1D IIR (v347 Candidato)"][0]
    cand_params = raw_results_data["Vectorized Selective-Conv1D IIR (v347 Candidato)"]["params"]
    ...
    ledger_entry = {
        ...
        "familia": "espectral / seleccional / vectorizacion",
        "dataset": "MQARDataset (2400 seqs / L=128 / Vectorized Scan)",
