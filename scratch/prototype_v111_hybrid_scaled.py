...
class TriWalshHybridScaled(nn.Module):
    def __init__(self, h_tri=96, h_walsh=96, k=16):
        super().__init__()
        self.tri_path = TriangularLayer(56, h_tri) 
        self.walsh_path = SpectralLayer(784, h_walsh, k=k)
...
def train_hybrid(train_loader, test_loader, epochs=10):
    ...
    # Con h=96 en ambas ramas:
    # Tri: 96*3 = 288
    # Walsh: 96*16 + 96 = 1632
    # Linear: 192*10 + 10 = 1930
    # Total: 3850 parámetros
    model = TriWalshHybridScaled(h_tri=96, h_walsh=96, k=16).to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    criterion = nn.CrossEntropyLoss()
    
    params = sum(p.numel() for p in model.parameters())
    print(f"Hybrid Scaled Model Parameters: {params}")
