...
class LogGatedAttentionLayer(nn.Module):
    def __init__(self, in_features, out_features, rank=2, mask_prob=0.5):
        ...
        # Log-Gated: M = exp(S). 
        # Inicializamos S pequeño para que exp(S) empiece cerca de 1.
    ...
    def forward(self, x):
        # S es el pre-gating en el espacio logarítmico
        s = torch.matmul(self.delta_in_s, self.delta_out_s)
        # M es estrictamente positivo, acotando problemas de signo
        w_m = torch.exp(s) 
        ...
        if self.training and self.mask_prob < 1.0:
            ...
            # En inferencia, el valor esperado.
...
class LogGatedAttentionMLP(nn.Module):
    def __init__(self, mask_prob=0.5):
        super().__init__()
        self.layer1 = LogGatedAttentionLayer(784, 512, rank=2, mask_prob=mask_prob)
        self.layer2 = LogGatedAttentionLayer(512, 10, rank=2, mask_prob=mask_prob)
...
def main():
    ...
    print(f"Benchmarking V2 (Log-Gated Attention) with ADAM on: {device}")
    ...
    model = LogGatedAttentionMLP(mask_prob=0.5).to(device)
