...
def run_experiment(name, optimizer_class, optimizer_kwargs, train_loader, test_loader, epochs=10, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        print(f"  Epoch {epoch+1}/{epochs} | Loss: {epoch_loss/len(train_loader):.4f} | Acc: {acc:.2f}%")
...
def main():
    ...
    # 1. Conservative PID (Low Ki, High Kd, WD)
    pid_res = run_experiment("Conservative PID (Kp=1, Ki=200, Kd=0.1, WD=0)", PID, {"lr": 1e-3, "kp": 1.0, "ki": 200.0, "kd": 0.1, "weight_decay": 0}, train_loader, test_loader, epochs=epochs, device=device)
    pid_res = run_experiment("Conservative PID (Kp=1, Ki=500, Kd=1, WD=1e-4)", PID, {"lr": 1e-3, "kp": 1.0, "ki": 500.0, "kd": 1.0, "weight_decay": 1e-4}, train_loader, test_loader, epochs=epochs, device=device)
    
    # 2. Adam Baseline (Standard with WD)
    # adam_res = run_experiment("Adam (Standard + WD)", optim.Adam, {"lr": 1e-3, "weight_decay": 1e-4}, train_loader, test_loader, epochs=epochs, device=device)
    
    # Summary
    print("\n" + "="*50)
    print("CIFAR-10 VALIDATION SUMMARY (TUNED)")
    print("="*50)
    print(f"PID Acc:  {pid_res['final_objective']:.2f}%")
    # print(f"Adam Acc: {adam_res['final_objective']:.2f}%")
    # print(f"Diff:     {pid_res['final_objective'] - adam_res['final_objective']:+.2f}%")
    
    # Save results
    # os.makedirs("results/raw", exist_ok=True)
    # timestamp = int(time.time())
    # with open(f"results/raw/cifar10_pid_v269_{timestamp}.json", "w") as f:
    #     json.dump([pid_res, adam_res], f, indent=4)
