...
# --- EXTRACTOR HÍBRIDO CON FILTRO AC (SIN MEDIA) ---
...
def get_hybrid_feats_ac(images_batch, batch_size=5000):
    """ Calcula firmas híbridas eliminando el componente DC para resaltar la forma """
    feats_list = []
    for i in range(0, images_batch.size(0), batch_size):
        ...
        # WALSH AC (Shape only)
        w = fwht(padded)
        w[:, 0] = 0 # ELIMINAMOS EL COMPONENTE DC (La media de intensidad)
        w_norm = F.normalize(w, p=2, dim=1)
        
        # ISLAS (Estructura)
        isl = F.normalize(get_island_signatures(batch), p=2, dim=1)
        
        feats_list.append(torch.cat([w_norm, isl], dim=1).cpu())
    return torch.cat(feats_list, dim=0)

def run_experiment():
    print(f"\n--- EXPERIMENTO V155: SPECTRAL NOVELTY (AC-ONLY) ---")
    ...
    # 2. APRENDIZAJE POR SORPRESA ESTRUCTURAL
    surprise_threshold = 0.85 # Umbral ajustado al espacio AC
    memory_bank = []
    memory_targets = []
    
    print(f"Construyendo memoria estructural (Umbral Sorpresa AC: {surprise_threshold})...")
    t0 = time.perf_counter()
    
    processed_count = 0
    for batch_imgs, batch_targets in train_loader:
        feats = get_hybrid_feats_ac(batch_imgs)
    ...
    # 3. EVALUACIÓN
    print("\nEvaluando resonancia sobre memoria estructural...")
    test_feats = get_hybrid_feats_ac(raw_test)
    ...
    print("\n" + "="*55)
    print(f"RESULTADO MEMORIA AC-ONLY (V155)")
    print(f"="*55)
