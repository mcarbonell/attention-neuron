...
class TriangularLayer(nn.Module):
    """
    Weights are synthesized using a triangular function.
    Params: 2 per neuron (center, width).
    """
...
class SpectralLayer(nn.Module):
    """
    Weights synthesized from a spectral core (DCT or Walsh).
    Params: k per neuron.
    """
    def __init__(self, in_features, out_features, k=16, mode='dct'):
        ...
        else: # Walsh (Sequency ordered approximation)
            def walsh_basis(n, i, freq):
                # Simple sequency-ordered walsh-like basis
                # (This is a simplified version for research purposes)
                # In a real scenario we'd use bit-reversal and Gray code
                return torch.sign(torch.cos(math.pi * freq * i / n))
...
class ComparisonNet(nn.Module):
    def __init__(self, neuron_type, in_dim, out_dim=10, h=32, k=16):
        super().__init__()
        if neuron_type == 'mlp':
...
def run_experiment(neuron_name, representation_name, input_key, in_dim, train_loader, test_loader, epochs=5):
    print(f"Running: {neuron_name} + {representation_name}...")
    device = torch.device("cpu")
    model = ComparisonNet(neuron_name, in_dim).to(device)
