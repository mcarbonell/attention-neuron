...
class GatedMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, init_val=1.0):
        ...
        self.relu = nn.ReLU()
        self.layer2 = GatedLinear(hidden_dim, output_dim, 1.0) # Last layer gates always 1.0
...
def calculate_pr(gate_tensor):
    # Formula: effective_n = (sum|g|)^2 / sum(g^2)
...
def run_trial(init_val, train_loader, test_loader, config):
    print(f"\n--- Running Trial with Gate Init = {init_val} ---")
    set_seed(config["seed"])
    model = GatedMLP(784, config["hidden_dim"], 10, init_val).to(config["device"])
    optimizer = optim.Adam(model.parameters(), lr=config["max_lr"]/10) # No WD
    ...
    return history, model.layer1.gate.detach().cpu().numpy()
...
def main():
    ...
    # Run trials
    h_one, g_one = run_trial(1.0, train_loader, test_loader, CONFIG)
    h_zero, g_zero = run_trial(0.0, train_loader, test_loader, CONFIG)
    ...
    plt.subplot(1, 2, 1)
    plt.plot(range(1, 11), h_one["acc"], label="Init=1.0 (Baseline)", marker='o')
    plt.plot(range(1, 11), h_zero["acc"], label="Init=0.0 (Discovery)", marker='s')
    ...
    # 2. Participation Ratio over time
    plt.subplot(1, 2, 2)
    plt.plot(range(1, 11), h_one["pr"], label="Init=1.0", marker='o')
    plt.plot(range(1, 11), h_zero["pr"], label="Init=0.0", marker='s')
    plt.title("Effective Number of Gates (Participation Ratio)")
    plt.xlabel("Epoch")
    ...
    plt.savefig("results/figures/v251l_oligarchy_analysis.png")
    print("\nOligarchy plot saved to: results/figures/v251l_oligarchy_analysis.png")
    
    # Final stats
    print(f"\n--- Final Results ---")
    print(f"Baseline (Init=1.0) Acc: {h_one['acc'][-1]:.2f}% | Final PR: {h_one['pr'][-1]:.2f}")
    print(f"Discovery (Init=0.0) Acc: {h_zero['acc'][-1]:.2f}% | Final PR: {h_zero['pr'][-1]:.2f}")
