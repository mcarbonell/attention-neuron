...
def get_data(func_name, n_samples=5000, train_range=(-10, 10), test_ranges=[(-20, 20), (-40, 40)]):
    """
    Genera datos para entrenamiento (interpolación) y múltiples tests de generalización.
    """
...
def run_experiment(func_name, model_type="mlp", epochs=3000):
    # Rango de entrenamiento más amplio y múltiples tests
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0:
            print(f"[{func_name} - {model_type}] Epoch {epoch}: Loss {loss.item():.8f}")
...
def main():
    ...
    print("\n" + "="*70)
    print("EXPERIMENTO V246b: EXPANDED RANGE AUGMENTED FEATURE APPROXIMATOR")
    print("Training: [-10, 10] | Test: [-20, 20], [-40, 40]")
    print("="*70 + "\n")
    ...
    for func in functions:
        ...
        res_aug = run_experiment(func, "augmented")
        ...
        print(f"  Aug-Neuron   | MSE Test(20): {res_aug['mse_test_20']:.2e} | MSE Test(40): {res_aug['mse_test_40']:.2e}")
        
        # Mostrar interpretación
        print("\n  Top Learned Weights (Interpretability):")
    ...
    with open("results/raw/v246b_augmented_features_wide.json", "w") as f:
        json.dump(json_results, f, indent=4)
        
    print(f"\nResultados guardados en results/raw/v246b_augmented_features_wide.json")
