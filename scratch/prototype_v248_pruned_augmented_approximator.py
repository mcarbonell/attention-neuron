...
def run_experiment(func_name, model_type="mlp", epochs=4000, lambda_l1=0.01, pruning_threshold=0.05):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 1000 == 0:
            print(f"[{func_name} - {model_type}] Epoch {epoch}: MSE {mse_loss.item():.8f}")
            
    # AGGRESSIVE PRUNING step
    if model_type == "augmented" and pruning_threshold > 0:
        print(f"  >>> Applying Hard Thresholding (threshold={pruning_threshold})")
        with torch.no_grad():
            # Prune weights
            mask = torch.abs(model.linear.weight) > pruning_threshold
            model.linear.weight *= mask
            # Optional: Prune bias
            if abs(model.linear.bias.item()) < pruning_threshold:
                model.linear.bias.zero_()
        
        # Optional Fine-tuning (just 100 epochs to stabilize the chosen bases)
        ...
        for _ in range(200):
            ...
            loss.backward()
            # Freeze the pruned weights (keep them at zero)
            with torch.no_grad():
...
def main():
    ...
    print("\n" + "="*70)
    print("EXPERIMENTO V248: PRUNED AUGMENTED FEATURE APPROXIMATOR")
    print("L1 Regularization + Hard Thresholding (0.05)")
    print("="*70 + "\n")
    ...
    for func in functions:
        ...
        res_aug = run_experiment(func, "augmented", lambda_l1=0.01, pruning_threshold=0.05)
        ...
        print(f"  Pruned-Aug   | MSE Test(20): {res_aug['mse_test_20']:.2e} | MSE Test(40): {res_aug['mse_test_40']:.2e}")
        
        print("\n  Learned Formula (Active Bases):")
        sorted_weights = sorted(res_aug["weights"], key=lambda x: abs(x[1]), reverse=True)
        active_bases = 0
        for name, val in sorted_weights:
            if abs(val) > 1e-6: # Absolute zero after pruning
                print(f"    {name:<12}: {val:10.4f}")
                active_bases += 1
        if abs(res_aug['bias']) > 1e-6:
    ...
    with open("results/raw/v248_pruned_augmented.json", "w") as f:
        json.dump(json_results, f, indent=4)
        
    print(f"\nResultados guardados en results/raw/v248_pruned_augmented.json")
