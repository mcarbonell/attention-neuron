...
class DeepGatedMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.layers = nn.Sequential(
            GatedLinear(input_dim, hidden_dim, frozen=True),
            nn.ReLU(),
            GatedLinear(hidden_dim, hidden_dim, frozen=True),
            nn.ReLU(),
            GatedLinear(hidden_dim, output_dim, frozen=True)
        )
...
def train_and_eval(hidden_dim, train_loader, test_loader, config):
    print(f"\n--- Testing Dimension: {hidden_dim} (2 Hidden Layers) ---")
    
    model = DeepGatedMLP(784, hidden_dim, 10).to(config["device"])
...
def main():
    ...
    with open("results/raw/v251c_deep_gating.json", "w") as f:
