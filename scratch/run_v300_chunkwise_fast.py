"""
run_v300_chunkwise_fast.py
==========================
Ultra-Fast Chunkwise Parallelized Version of V300 Capacity Scaling Benchmark.
Employs Householder WY Representation (NeurIPS 2024) Chunkwise Parallel Blocks:
1. ChunkwiseComplexDeltaPhaseBlock (Complex U(1) phase O(N)).
2. ChunkwiseRealDeltaNetBlock (Real square state memory).
3. ChunkwiseRealDeltaNetRectangularBlock (Real rectangular state memory - Iso-Rectangular control).
4. CausalAttentionBlock (Softmax MHA baseline).

Enriched Metadata Header + GPU Vectorized Dataset Generation.
Runs complete V300 sweep in ~4-5 minutes on T4 GPU (~30-35 mins on CPU).
"""
...
CFG = {
    "exp_id": "v300_capacity_scaling_chunkwise_fast",
    "d_k_list": [64, 128],
    "iso_floats_map": {
...
# ── 1. Vectorized Dataset Generator ─────────────────────────────────────
...
def train_and_eval_pregenerated(model, model_name, train_x, train_y, eval_x, eval_y, lr, epochs=15, steps_per_epoch=50, seq_len=256):
    ...
    for ep in range(epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            optimizer.step()
        print(f"      [{model_name:38s} | lr={lr:.4f}] Epoch {ep+1:2d}/15 Complete | Loss = {total_loss:.4f}", flush=True)
...
print("=" * 85)
print(f"EXPERIMENT BENCHMARK RUN: V300 CHUNKWISE PARALLEL CAPACITY SCALING")
print("=" * 85)
...
print(f"  * Chunk Size C:    {CFG['chunk_size']}")
...
for d_k in CFG["d_k_list"]:
    ...
    for num_pairs in CFG["num_pairs_list"]:
        ...
        for name, block_cls, block_kwargs in model_specs:
            ...
            print(f"      [{name:38s} | d_k={d_k_key}] Pairs={num_pairs:3d} (L={seq_len:4d}) -> Best Acc: {best_acc:6.2f}% (lr={best_lr})", flush=True)

with open("v300_chunkwise_capacity_scaling_results.json", "w") as f:
    json.dump({"config": CFG, "results": results_matrix}, f, indent=2)
print("\nBENCHMARK COMPLETE! Results saved to v300_chunkwise_capacity_scaling_results.json", flush=True)
