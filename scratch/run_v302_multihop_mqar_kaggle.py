# ============================================================================
# V302 MULTI-HOP MQAR BENCHMARK — Kaggle/Colab Single-Cell Version
# ============================================================================
# CONFIGURE HERE: "lite" (~15 min on T4) or "normal" (~4-6h on T4)
MODE = "normal"
SEED = 42
# ============================================================================
...
def compute_seq_len(n_chains, n_hops, chunk_size=64):
    ...
    min_len = 2 * total_pairs + 2 + 2 * n_chains
...
def generate_multihop_batch(batch_size, n_chains, n_hops, seq_len,
    ...
    assert tokens_needed <= num_tokens, (
        f"Need {tokens_needed} tokens for {n_chains} chains x {n_hops} hops, "
    ...
    pair_keys = chain_tokens[:, :, :-1].reshape(batch_size, -1)
    ...
    x[:, :2 * total_pairs] = kv
    query_starts = chain_tokens[:, :, 0]
    query_answers = chain_tokens[:, :, -1]
...
print("=" * 85)
print(f"{ts()} EXPERIMENT: V302 MULTI-HOP MQAR BENCHMARK ({MODE.upper()})")
print("=" * 85)
...
output_file = f"v302_multihop_mqar_{MODE}_results.json"
