...
def create_spectrum_kernels(out_channels, in_channels, kernel_size=3, device='cpu'):
    ...
    def batch_perlin(scale):
        grid_h = max(2, int(kernel_size * scale))
        grid_w = max(2, int(kernel_size * scale))
        grads = torch.randn(out_channels, in_channels, grid_h, grid_w, 2, device=device)
        grads = grads / (grads.norm(dim=-1, keepdim=True) + 1e-8)
        y = torch.linspace(0, grid_h - 1, kernel_size, device=device)
        x = torch.linspace(0, grid_w - 1, kernel_size, device=device)
        gy = torch.floor(y).long()
        gx = torch.floor(x).long()
        fy = (y - gy).unsqueeze(1)
        fx = (x - gx).unsqueeze(0)
        ...
        ly = fy.expand(kernel_size, kernel_size)
        lx = fx.expand(kernel_size, kernel_size)
        
        g00 = grads[:, :, gy0[:, None].expand(kernel_size, kernel_size), gx0[None, :].expand(kernel_size, kernel_size)]
        d00 = g00[..., 0] * lx + g00[..., 1] * ly
        g10 = grads[:, :, gy0[:, None].expand(kernel_size, kernel_size), gx1[None, :].expand(kernel_size, kernel_size)]
        d10 = g10[..., 0] * (lx - 1) + g10[..., 1] * ly
        g01 = grads[:, :, gy1[:, None].expand(kernel_size, kernel_size), gx0[None, :].expand(kernel_size, kernel_size)]
        d01 = g01[..., 0] * lx + g01[..., 1] * (ly - 1)
        g11 = grads[:, :, gy1[:, None].expand(kernel_size, kernel_size), gx1[None, :].expand(kernel_size, kernel_size)]
        d11 = g11[..., 0] * (lx - 1) + g11[..., 1] * (ly - 1)
        
        sx_exp = sx.expand(kernel_size, kernel_size)
        sy_exp = sy.expand(kernel_size, kernel_size)
        
        n0 = d00 * (1 - sx_exp) + d10 * sx_exp
        n1 = d01 * (1 - sx_exp) + d11 * sx_exp
        noise = n0 * (1 - sy_exp) + n1 * sy_exp
        return noise

    # 1. Perlin Low Freq
    k_perlin_low = batch_perlin(0.5)
    k_perlin_low = k_perlin_low / (k_perlin_low.std() + 1e-8) * std
    kernels.append(k_perlin_low)
    
    # 2. Perlin Med Freq
    k_perlin_med = batch_perlin(1.5)
    ...
    x = torch.arange(kernel_size, device=device).view(1, -1).expand(kernel_size, kernel_size).float()
    y = torch.arange(kernel_size, device=device).view(-1, 1).expand(kernel_size, kernel_size).float()
