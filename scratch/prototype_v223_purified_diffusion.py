"""
scratch/prototype_v223_purified_diffusion.py - Purified Spectral Diffusion (MNIST)

Experimento V223:
Aplica la logica PAC (Purifying Archetype Classifier) a la difusion espectral.
Cada digito tiene su propio "experto" que solo aprende a de-noisar su propia clase.
Esto evita la contaminacion de gradientes y el difuminado discriminativo, permitiendo
generar arquetipos "puros".
"""
...
LR = 0.001
EPOCHS = 10 
SPECTRAL_SCALE = 0.1 # Factor sugerido en V222 para equilibrar señal y ruido
...
class ExpertDenoiseNet(nn.Module):
    """Un experto especializado en una sola clase de arquetipo."""
...
class PurifiedDenoiseBank(nn.Module):
    ...
    def forward(self, x, t, labels):
        ...
        # En entrenamiento, cada ejemplo pasa por SU experto
        # Usamos un bucle para manejar el batch (o un scatter/gather si fuera masivo, pero 10 es manejable)
        preds = torch.zeros_like(x)
        for i in range(10):
            mask = (labels == i)
            if mask.any():
                preds[mask] = self.experts[i](x[mask], t_embed[mask])
        return preds
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            # 1. Transformar a DCT y ESCALAR (PAC Logic)
            with torch.no_grad():
            ...
            # 4. Predecir ruido (Selective Backprop via class expert)
            optimizer.zero_grad()
    ...
    # --- SAMPLING (PURIFIED REVERSE DIFFUSION) ---
    model.eval()
    print("\nGenerating Pure Archetypes for each class...")
    
    final_samples = []
    with torch.no_grad():
        for target_label in range(10):
            # 16 muestras del mismo experto
            ...
            for i in reversed(range(0, TIMESTEPS)):
                ...
                # Usar solo el experto seleccionado (Pure Archetype)
            ...
            # Revertir escalado y transformar a pixeles
    ...
    plt.tight_layout()
    plt.savefig("results/v223_purified_archetypes.png")
    print("Samples saved to results/v223_purified_archetypes.png")
    ...
    metrics = {
        ...
        "method": "PAC-Purified-Spectral-Diffusion"
    }
    
    with open("results/raw/v223_results.json", "w") as f:
