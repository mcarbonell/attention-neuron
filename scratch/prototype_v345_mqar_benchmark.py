...
def train_model(name, model, train_loader, val_dataset, epochs=25, lr=2e-3, device='cpu'):
...
def run_experiment_v345():
    log_print("================================================================================")
    log_print(" EXPERIMENTO V345: BENCHMARK MQAR ESTÁNDAR (ZOOLOGY/H3/ANTHROPIC CIRCUITS)")
    log_print("================================================================================")
    ...
    log_print(f"ID Experimento: v345")
    ...
    models = {
        "Selective-Conv1D IIR (v345 Candidato)": SelectiveConv1DIIRTransformerV345(
            vocab_size=config["vocab_size"],
        ...
        "Dynamic IIR Filter (Baseline)": DynamicIIRTransformer(
            vocab_size=config["vocab_size"],
            d_model=config["d_model"]
        )
    }
    ...
    log_print("--- FASE 1: ENTRENAMIENTO MQAR EN L = 128 ---")
    for name, model in models.items():
        log_print(f"\n[Entrenando] {name}...")
        res = train_model(
    ...
    for L_eval in config["eval_lengths"]:
        ...
        for name, data in trained_results.items():
            ...
            log_print(f"   * {name:48s} | Acc L={L_eval:4d}: {acc:6.2f}%")

    log_print("\n================================================================================")
    log_print("          RESUMEN EXPERIMENTO V345 (BENCHMARK MQAR ESTÁNDAR)")
    log_print("================================================================================")
    log_print(f"{'Modelo':48s} | {'L=128 (Train)':13s} | {'L=256':9s} | {'L=512':9s}")
    log_print("-" * 85)
    
    raw_results_data = {}
    for name in models.keys():
        accs = eval_results[name]
        log_print(f"{name:48s} | {accs[0]:12.2f}% | {accs[1]:8.2f}% | {accs[2]:8.2f}%")
    ...
    json_path = os.path.join(results_dir, "v345_results.json")
    ...
    # LEDGER
    cand_acc = eval_results["Selective-Conv1D IIR (v345 Candidato)"][0]
    cand_params = raw_results_data["Selective-Conv1D IIR (v345 Candidato)"]["params"]
    ...
    ledger_entry = {
        ...
        "familia": "espectral / seleccional / mqar",
        "dataset": "MQARDataset (2000 seqs / L=128 / 8 pairs)",
        "n_eval": 2000 * config["epochs"],
