"""
prototype_v293_holographic_mqar.py
===================================
V293: Holographic Phase Conjugation (HRR) for Associative Recall (MQAR).

Tests whether Complex Phase Binding & Unbinding via Phase Conjugation:
  Binding  : B_t = exp(i * theta_k(x_t)) * V_t
  Memory   : M_t = CausalCumSum(B_t)  [or CausalFFT(B_t)]
  Unbinding: R_t = Re( conj(exp(i * theta_q(x_t))) * M_t )
can achieve 100% Associative Recall (MQAR) in O(N) / O(N log N) time without O(N^2) Softmax Attention!

Models compared (~96k - 108k params):
  1. CausalHolographicAccumulator [CANDIDATE 1]: O(N) Causal cumsum phase memory + Conjugate unbinding
  2. CausalHolographicFFTMixer    [CANDIDATE 2]: O(N log N) Zero-padded Causal FFT + Conjugate unbinding
  3. CausalGatedFFTMixer          [BASELINE 1 - Negative v292]: Element-wise gating (gave ~2.81%)
  4. CausalAttentionMHA           [BASELINE 2 - Quadratic]: Softmax MHA O(N^2)

Methodology:
  - MQAR synthetic task: L=64, num_kv_pairs=8, vocab=32 keys / 32 vals
  - Strict query target accuracy on held-out test batch
  - Results logged to results/raw/v293_holographic.json & results/master_ledger.jsonl
"""
...
# ── Candidate 1: Causal Holographic Accumulator (HRR O(N)) ──────────────
class CausalHolographicAccumulatorBlock(nn.Module):
    """
    Holographic Memory via Causal CumSum and Phase Conjugate Unbinding.
    Binding: B_t = exp(i * theta_k) * V_t (complex tensor)
    Memory : M_t = CumSum_{tau <= t}(B_tau)  [fully vectorized!]
    Unbind : R_t = Re( conj(exp(i * theta_q)) * M_t )
    """
    ...
    def forward(self, x):
        ...
        # 1. Phasors theta_k and theta_q in [0, 2*pi]
        theta_k = self.theta_k_proj(normed)
        theta_q = self.theta_q_proj(normed)
        
        # Unit complex phasors: K = exp(i * theta_k), Q = exp(i * theta_q)
        K = torch.polar(torch.ones_like(theta_k), theta_k) # [B, L, D] complex
        Q = torch.polar(torch.ones_like(theta_q), theta_q) # [B, L, D] complex
        
        v = self.val_proj(normed) # [B, L, D] real
        
        # 2. Binding: B_t = K_t * V_t
        bound = K * v # [B, L, D] complex
        
        # 3. Causal Memory Accumulation: M_t = sum_{tau <= t} B_tau
        memory = torch.cumsum(bound, dim=1) # [B, L, D] complex
        
        # 4. Unbinding via Phase Conjugate: R_t = Re( conj(Q_t) * M_t )
        unbound_complex = torch.conj(Q) * memory
        retrieved = unbound_complex.real # [B, L, D] real
        
        x = res + self.out_proj(retrieved)
        x = x + self.ffn(self.norm2(x))
        return x

# ── Candidate 2: Causal Holographic FFT Mixer Block ────────────────────
class CausalHolographicFFTMixerBlock(nn.Module):
    """
    Holographic Memory via Causal FFT Convolution and Phase Conjugate Unbinding.
    Zero-pads bound sequence (2L) -> Causal FFT -> Inverse FFT -> Conjugate Unbind.
    """
    def __init__(self, d_model):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
    ...
    def forward(self, x):
        ...
        # Causal Zero-padding for complex FFT
        padded = F.pad(bound, (0, 0, 0, L)) # [B, 2L, D] complex
        fft_feat = torch.fft.fft(padded, dim=1)
        ifft_feat = torch.fft.ifft(fft_feat, dim=1)[:, :L, :]
        
        unbound = torch.conj(Q) * ifft_feat
...
# ── Baseline 1: Causal Gated FFT Mixer (Negative from v292) ────────────
class CausalGatedFFTMixerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        v = self.val_proj(normed)
        padded = F.pad(v * g, (0, 0, 0, L))
        fft_feat = torch.fft.rfft(padded, dim=1)
        ifft_feat = torch.fft.irfft(fft_feat, n=2*L, dim=1)[:, :L, :]
...
class MQARModel(nn.Module):
    def __init__(self, block_cls, vocab_size, d_model, n_layers, seq_len=64):
        super().__init__()
...
# ── Main ────────────────────────────────────────────────────────────────
def main():
    print(f"Starting V293 Holographic Phase Conjugation Benchmark on device: {device}")
    ...
    # Candidate 1: Causal Holographic Accumulator (HRR O(N))
    res_holo_acc = run_model_benchmark("CausalHolographicAccumulator (Candidate 1 - HRR O(N))", CausalHolographicAccumulatorBlock, CFG)
    results.append(res_holo_acc)
    
    # Candidate 2: Causal Holographic FFT Mixer (HRR O(N log N))
    res_holo_fft = run_model_benchmark("CausalHolographicFFTMixer (Candidate 2 - HRR O(N log N))", CausalHolographicFFTMixerBlock, CFG)
    results.append(res_holo_fft)
    
    # Baseline 1: Causal Gated FFT Mixer (Negative from v292)
    res_gated_fft = run_model_benchmark("CausalGatedFFTMixer (Baseline 1 - Negative v292)", CausalGatedFFTMixerBlock, CFG)
    results.append(res_gated_fft)
    ...
    raw_path = "results/raw/v293_holographic.json"
    ...
    with open(ledger_path, "a") as f:
        for r in results:
            entry = {
                "experiment_id": "v293_holographic",
                "fecha": time.strftime("%Y-%m-%d"),
                "familia": "holografico_fase vs attention",
