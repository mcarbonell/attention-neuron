...
from src.models.vectorized_gated_iir_v348 import VectorizedGatedIIRTransformerV348
...
def train_model(name, model, train_loader, val_dataset, epochs=35, lr=3e-3, device='cpu'):
...
def run_experiment_v348():
    log_print("================================================================================")
    log_print(" EXPERIMENTO V348: ALCANCE MÁXIMO ACCURACY MQAR (VECTORIZED GATED IIR 6-LAYERS)")
    log_print("================================================================================")
    ...
    # REGLA DE ORO (Primero el Candidato Vectorizado v348):
    models = {
        "Vectorized Gated IIR 6-Layers (v348 Candidato d_model=256)": VectorizedGatedIIRTransformerV348(
            vocab_size=config["vocab_size"],
    ...
    log_print("--- FASE 1: ENTRENAMIENTO MQAR EN L = 128 (35 ÉPOCAS) ---")
    for name, model in models.items():
        log_print(f"\n[Entrenando] {name}...")
        res = train_model(
    ...
    for L_eval in config["eval_lengths"]:
        ...
        for name, data in trained_results.items():
            ...
            log_print(f"   * {name:65s} | Acc L={L_eval:4d}: {acc:6.2f}%")

    log_print("\n================================================================================")
    log_print("          RESUMEN EXPERIMENTO V348 (ALCANCE MÁXIMO ACCURACY MQAR)")
    log_print("================================================================================")
    log_print(f"{'Modelo':65s} | {'Wall Clock (s)':14s} | {'L=128 (Train)':13s} | {'L=256':9s} | {'L=512':9s}")
    log_print("-" * 115)
    ...
    for name in models.keys():
        ...
        log_print(f"{name:65s} | {wall_t:13.2f}s | {accs[0]:12.2f}% | {accs[1]:8.2f}% | {accs[2]:8.2f}%")
    ...
    # LEDGER
    cand_acc = eval_results["Vectorized Gated IIR 6-Layers (v348 Candidato d_model=256)"][0]
    cand_params = raw_results_data["Vectorized Gated IIR 6-Layers (v348 Candidato d_model=256)"]["params"]
    ...
    ledger_entry = {
        ...
        "familia": "espectral / seleccional / vectorizacion_gated",
        "dataset": "MQARDataset (3200 seqs / L=128 / 6-Layers d_model=256)",
