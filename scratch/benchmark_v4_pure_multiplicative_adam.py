...
class PureMultiplicativeLayer(nn.Module):
    def __init__(self, in_features, out_features, rank=2, mask_prob=0.5):
        ...
        self.delta_in_m = nn.Parameter(torch.ones(out_features, rank) * rank_scale)
        self.delta_out_m = nn.Parameter(torch.ones(rank, in_features) * rank_scale)
    ...
    def forward(self, x):
        ...
        if self.training and self.mask_prob < 1.0:
            ...
            w_evolved = torch.where(mask > 0, self.w_init * w_m, self.w_init)
        else:
            m_eff = 1.0 + self.mask_prob * (w_m - 1.0)
            w_evolved = self.w_init * m_eff
            
        return torch.matmul(x, w_evolved.t()) + torch.sin(self.theta_bias)

class PureMultiplicativeMLP(nn.Module):
    def __init__(self, mask_prob=0.5):
        super().__init__()
        self.layer1 = PureMultiplicativeLayer(784, 512, rank=2, mask_prob=mask_prob)
        self.layer2 = PureMultiplicativeLayer(512, 10, rank=2, mask_prob=mask_prob)
...
def main():
    ...
    print(f"Benchmarking V4 (Pure Multiplicative) with ADAM on: {device}")
    ...
    model = PureMultiplicativeMLP(mask_prob=0.5).to(device)
