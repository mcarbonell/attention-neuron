...
class GatedMLP_Bias(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, init_val=0.0):
        ...
        # UNFREEZE the bias of the second layer
        self.layer2.linear.bias.requires_grad = True
...
def main():
    ...
    model = GatedMLP_Bias(784, CONFIG["hidden_dim"], 10, 0.0).to(CONFIG["device"])
    
    # Count trainable parameters to verify
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Total Trainable Parameters: {trainable_params}")
    # layer1.gate (4096) + layer2.gate (10) + layer2.bias (10) = 4116
    ...
    print("\n--- Running Trial (SiLU + Trainable Output Bias) with Gate Init = 0.0 ---")
