...
# --- The 938-Parameter Wonder ---

class NanoWalshNet(nn.Module):
    """
    V40: The Nano-Walsh Net.
    A deep neural network with less than 1024 parameters total.
    Uses 3 layers of FWHT modulation interleaved with Spatial ReLUs,
    and a heavily pooled 4x4 final classification layer.
    """
    def __init__(self, num_bands=128):
        super().__init__()
        self.N = 1024 # Padded 32x32
        self.num_bands = num_bands
        self.band_size = self.N // num_bands # 8 frequencies per band
    ...
    def forward(self, x):
        ...
        # 4. Classify (Zero Hidden Layers, just a projection)
        out = self.fc(pooled.view(B, 16))
        return out

def main():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Training V40 'NANO-WALSH NET' on: {device}")
    
    BATCH_SIZE = 256
    EPOCHS = 15
    LR = 0.02 # High LR since parameters are so few
    ...
    model = NanoWalshNet(num_bands=128).to(device)
