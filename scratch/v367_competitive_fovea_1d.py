...
# -----------------------------------------------------------------------------
# Dataset Sintético: MQAR 1D (T=128, 4 parejas K-V)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Core Layer: CompetitiveFoveaAttention1D (Opción A: MaxPool | Opción B: Softmax Gating)
# -----------------------------------------------------------------------------
class CompetitiveFoveaAttention1D(nn.Module):
    def __init__(self, d_model=64, num_heads=16, mode='maxpool'):
        super().__init__()
        self.d_model = d_model
        self.num_heads = num_heads
        self.mode = mode # 'maxpool' o 'gated'
        ...
        if mode == 'gated':
            self.gate_predictor = nn.Linear(d_model, num_heads)
    ...
    def forward(self, x):
        ...
        v = self.v_proj(x).view(B, T, K, -1).permute(0, 2, 1, 3) # [B, K, T, d_head]
        
        attn_out_heads = torch.matmul(head_weights, v) # [B, K, T, d_head]
        
        if self.mode == 'maxpool':
            # OPCIÓN A: Max-Pooling competitivo entre cabezas (Winner-Takes-All)
            attn_pooled = attn_out_heads.max(dim=1)[0] # [B, T, d_head]
            if d_head != D:
                # Proyección a D
                attn_pooled = attn_pooled.repeat(1, 1, D // d_head)
            out = attn_pooled
        elif self.mode == 'gated':
            # OPCIÓN B: Gating Softmax entre cabezas (peso cero a cabezas inactivas)
            gates = F.softmax(self.gate_predictor(x), dim=-1) # [B, T, K]
            gates_exp = gates.permute(0, 2, 1).unsqueeze(-1) # [B, K, T, 1]
            attn_gated = (attn_out_heads * gates_exp).sum(dim=1) # [B, T, d_head]
            if d_head != D:
                attn_gated = attn_gated.repeat(1, 1, D // d_head)
            out = attn_gated
...
# Modelo
# -----------------------------------------------------------------------------
class CompetitiveFovea1DNet(nn.Module):
    def __init__(self, vocab_size, d_model=64, num_heads=16, mode='maxpool'):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, d_model)
        self.pos_emb = nn.Embedding(256, d_model)
        
        self.fovea_attn = CompetitiveFoveaAttention1D(d_model=d_model, num_heads=num_heads, mode=mode)
...
# -----------------------------------------------------------------------------
# Bucle Principal Experimento V367
# -----------------------------------------------------------------------------
def run_experiment():
    log_msg("=========================================================================")
    log_msg("  EXPERIMENTO V367: Selección Competitiva Fóvea 1D (MaxPool vs Gated)")
    log_msg("=========================================================================")
    ...
    modes = ['maxpool', 'gated']
    head_configs = [4, 16, 64]
    epochs = 12
    v367_results = {}
    
    for mode in modes:
        for K in head_configs:
            exp_name = f"Mode_{mode}_K_{K}"
            log_msg(f"\n>>> Evaluando Configuración: Modo = {mode.upper()} | K = {K} Cabezas <<<")
            torch.manual_seed(seed)
            
            model = CompetitiveFovea1DNet(vocab_size=vocab_size, d_model=64, num_heads=K, mode=mode).to(device)
            ...
            for epoch in range(1, epochs + 1):
                ...
                for batch_idx, (seqs, targets) in enumerate(train_loader):
                    ...
                    if epoch == 1 and batch_idx == 0:
                        log_msg(f"  [FAST-FEEDBACK {mode} K={K}] Epoch 1 Batch 1 Loss: {loss.item():.4f}")
                ...
                log_msg(f"  [{mode.upper()} K={K:02d}] Epoch {epoch:02d}/{epochs:02d} | Acc: {acc:.2f}% | Tiempo: {t_epoch:.2f}s")
                history.append({"epoch": epoch, "acc": acc})
                
            pei = history[-1]['acc'] / math.log10(total_params + 1)
            v367_results[exp_name] = {
                "mode": mode,
                "num_heads": K,
                "params": total_params,
            ...
            log_msg(f"=== RESULTADO {mode.upper()} K={K} -> Acc Final: {history[-1]['acc']:.2f}% | PEI: {pei:.2f} | Params: {total_params} ===")

    # Tabulación Final
    log_msg("\n=========================================================================")
    log_msg("  RESUMEN FINAL EXPERIMENTO V367 (MAX-POOL VS SOFTMAX GATED)")
    log_msg("=========================================================================")
    log_msg(f"{'Modo':<10} | {'Cabezas (K)':<12} | {'Parámetros':<12} | {'Test Acc (%)':<15} | {'PEI':<10}")
    log_msg("-" * 65)
    for exp_name, res in v367_results.items():
        log_msg(f"{res['mode'].upper():<10} | {res['num_heads']:<12} | {res['params']:<12} | {res['final_acc']:<15.2f} | {res['pei']:<10.2f}")

    # Persistir JSON
    os.makedirs("results/raw", exist_ok=True)
    res_path = "results/raw/v367_copy_section_results.json"
    with open(res_path, "w") as f:
        json.dump(v367_results, f, indent=2)
    log_msg(f"Resultados guardados en: {res_path}")

    # Ledger
    best_key = max(v367_results.keys(), key=lambda k: v367_results[k]["final_acc"])
    best_res = v367_results[best_key]
    ...
    log_msg(f"Registrado en Master Ledger (Mejor {best_key}): {ledger_path}")
