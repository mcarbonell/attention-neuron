...
class DeltaPhaseMQARModel(nn.Module):
    """
    Modelo DeltaPhase adaptado para clasificación/recall en MQAR.
    Combina Embedding + Bloques DeltaPhase Holográficos (Fase Compleja C^(d_k x d_k)) + Head.
    """
    def __init__(self, vocab_size: int = 64, d_model: int = 128, n_heads: int = 4, num_layers: int = 2):
        super().__init__()
...
def run_experiment_deltaphase():
    log_print("================================================================================")
    log_print(" EXPERIMENTO DELTAPHASE: EVALUACIÓN EN ARNÉS MQAR ESTÁNDAR DE LA LITERATURA")
    log_print("================================================================================")
    ...
    log_print(f"ID Experimento: v349_deltaphase")
    ...
    models = {
        "DeltaPhase Holographic Core (Complex Phase Matrix State C^(32x32))": DeltaPhaseMQARModel(
            vocab_size=config["vocab_size"],
    ...
    log_print("\n================================================================================")
    log_print("          RESUMEN EXPERIMENTO DELTAPHASE MQAR BENCHMARK")
    log_print("================================================================================")
    log_print(f"{'Modelo':70s} | {'Wall Clock (s)':14s} | {'L=128 (Train)':13s} | {'L=256':9s} | {'L=512':9s}")
    log_print("-" * 115)
    ...
    for name in models.keys():
        ...
        log_print(f"{name:70s} | {wall_t:13.2f}s | {accs[0]:12.2f}% | {accs[1]:8.2f}% | {accs[2]:8.2f}%")
    ...
    json_path = os.path.join(results_dir, "v349_deltaphase_results.json")
