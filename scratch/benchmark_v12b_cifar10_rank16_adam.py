...
class ResidualAttentionLayer(nn.Module):
    def __init__(self, in_features, out_features, rank=16, mask_prob=0.5):
        super().__init__()
...
class HybridAttentionConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, rank=16, mask_prob=0.5):
        super().__init__()
...
class HybridAttentionCIFAR10_Rank16(nn.Module):
    def __init__(self, mask_prob=0.5):
        super().__init__()
        # Input: 3 x 32 x 32
        self.conv1 = HybridAttentionConv2d(3, 32, kernel_size=3, padding=1, rank=16, mask_prob=mask_prob)
        self.pool1 = nn.MaxPool2d(2) # -> 32 x 16 x 16
        
        self.conv2 = HybridAttentionConv2d(32, 64, kernel_size=3, padding=1, rank=16, mask_prob=mask_prob)
        self.pool2 = nn.MaxPool2d(2) # -> 64 x 8 x 8
        
        self.conv3 = HybridAttentionConv2d(64, 128, kernel_size=3, padding=1, rank=16, mask_prob=mask_prob)
        self.pool3 = nn.MaxPool2d(2) # -> 128 x 4 x 4
        
        # Clasificador
        self.fc = ResidualAttentionLayer(128 * 4 * 4, 10, rank=16, mask_prob=mask_prob)
...
def main():
    ...
    print(f"Benchmarking V12b (Hybrid Attention CNN, Rank=16) on CIFAR-10 with ADAM on: {device}")
    ...
    model = HybridAttentionCIFAR10_Rank16(mask_prob=0.5).to(device)
