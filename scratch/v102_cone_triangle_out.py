...
class TriangleAttention1DLayer(nn.Module):
    # Capa 2: Obligamos a la red a organizar las características topológicamente en 1D
    def __init__(self, in_features, out_features):
        ...
        # Inicializamos positivo por defecto para la capa final
...
class ConeTriangleNet(nn.Module):
    def __init__(self, hidden_size=256):
        ...
        # Capa 2: Triángulo 1D (Clasificación final forzando topología)
...
def train():
    ...
    model = ConeTriangleNet(hidden_size=256).to(device)
    
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Total parámetros entrenables: {total_params} !!!")
    
    # Mantenemos el LR en 0.001 (el exitoso del V101)
