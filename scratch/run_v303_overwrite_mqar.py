"""
run_v303_overwrite_mqar.py
==========================
V303 Overwrite MQAR Benchmark: Tests the Delta Rule erase/overwrite mechanism
  M_t = M_{t-1} + beta * (v_new - M_{t-1} @ k^*) @ k^*^T

In Overwrite MQAR, a fraction of keys are updated with new values later in the sequence.
Tests if Complex Phase interference provides cleaner memory erasure (v_old retrieval)
without leaving "phantom" residual noise in M compared to Real-valued controls.
...
Usage:
  python run_v303_overwrite_mqar.py --mode lite
  python run_v303_overwrite_mqar.py --mode normal --device cuda --seed 42
"""
...
# ── CLI ──────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Overwrite MQAR benchmark for DeltaPhase")
...
# ── 1. Overwrite MQAR Data Generator (vectorized GPU) ───────────────────
...
def generate_overwrite_batch(batch_size, n_keys, overwrite_ratio, seq_len,
    ...
    """
    Generate a batch of Overwrite MQAR data.
    
    1. n_keys distinct keys K_1..K_n and initial values V_{i,old}.
    2. k = round(n_keys * overwrite_ratio) keys are selected for overwrite.
    3. New values V_{j,new} assigned to overwritten keys.
    4. Sequence contains initial pairs, then overwrite pairs later.
    5. Query target for overwritten keys is V_{new}, for others V_{old}.
    """
    n_overwrites = int(round(n_keys * overwrite_ratio))
    total_pairs = n_keys + n_overwrites
    tokens_needed = n_keys + n_keys + n_overwrites  # keys + initial vals + overwrite vals
    
    assert tokens_needed <= num_tokens, (
        f"Need {tokens_needed} tokens, but num_tokens={num_tokens}"
    )
    
    # Sample distinct tokens for this batch
    ...
    # Build initial KV pairs
    init_k = keys
    init_v = vals_old
    
    # Shuffle initial pairs order
    ...
    # Overwrite pairs (first n_overwrites keys of the shuffled keys)
    targets = init_v.clone()
    if n_overwrites > 0:
        ow_k = init_k[:, :n_overwrites]
        ow_v = vals_new[:, :n_overwrites]
        
        # Shuffle overwrite pairs among themselves
        ...
        # Update targets for overwritten keys
    ...
    # Query Section (query all n_keys in shuffled order)
...
print("=" * 85)
print(f"{ts()} EXPERIMENT: V303 OVERWRITE MQAR BENCHMARK ({args.mode.upper()})")
print("=" * 85)
...
output_file = f"v303_overwrite_mqar_{args.mode}_results.json"
